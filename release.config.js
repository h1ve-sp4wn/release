...
console.info(`Executing semantic-release config setup`);
...
const addPlugin = (plugin, options) => {
  console.info(`${plugin} enabled with${options && ' options:' || 'out options'}`);
  options && console.info(null, options);
  return releaseConfig.plugins.push([plugin, options]);
};

!GIT_COMMITTER_NAME && (process.env.GIT_COMMITTER_NAME = "h1ve-sp4wn[bot]");
!GIT_COMMITTER_EMAIL && (process.env.GIT_COMMITTER_EMAIL = "194594720+h1ve-sp4wn[bot]@users.noreply.github.com");
...
try {
  ...
  console.warn(`Unable to set GIT_COMMITTER_NAME and GIT_COMMITTER_EMAIL`);
  console.error(e);
}

console.info(`Adding semantic-release config plugins`);
...
if (actionExists) {
  ...
  if (!imageTag) {
    console.warn(`Unable to find image tag in action.yml, no action will be published to the GitHub Container Registry`);
    console.warn(`Please add the following to your action.yml file:`);
    console.warn(`image: 'docker://ghcr.io/${owner}/${repo}:latest'`);
  }
